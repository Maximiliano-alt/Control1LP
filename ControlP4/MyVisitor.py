...
class MyVisitor(CuartaParteVisitor):
    ...
    # Logica para la repeticion de los comandos de movimiento
    def visitSntxRepMov(self, ctx):
        # Almacenado de la cantidad de repeticiones ingresadas
        rep = int(ctx.INT().getText())
        x = 0

        # Repeticion del comando ingresado segun la cantidad de repeticiones ingresadas
        while x < rep:
            self.visit(ctx.movimiento())
            x += 1
        return 0
    
    # Logica para la repeticion de los comandos de rotacion
